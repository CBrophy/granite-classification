...
public class BayesModelTest {
  ...
  public void testMeanScoreObservation() throws Exception {
    ...
    // P(H:B) = P(H) * P(B:H) / P(B)
    double hBPostProb =
        (hProb * stats.getAssociatedValueProbabilities().getOrDefault("h", 0.0)) / bProb;
    double cBPostProb =
        (cProb * stats.getAssociatedValueProbabilities().getOrDefault("c", 0.0)) / bProb;
    double cAPostProb =
        (cProb * aStats.getAssociatedValueProbabilities().getOrDefault("c", 0.0)) / aProb;
    double hAPostProb =
        (hProb * aStats.getAssociatedValueProbabilities().getOrDefault("h", 0.0)) / aProb;
    double bHPostProb =
        (bProb * hStats.getAssociatedValueProbabilities().getOrDefault("b", 0.0)) / hProb;
    ...
    double bBPostProb =
        (bProb * stats.getAssociatedValueProbabilities().getOrDefault("b", 0.0)) / bProb;
